...
def main():
    ...
    print("[2/4] Confluence 원본 가져오는 중...")
    title, html = fetch_page(page_id)
    print("      제목: " + title)
    attachments = fetch_attachments(page_id)
    glossary = load_glossary()

    print("[3/4] 매크로 보호 처리 중...")
    protected_html, placeholders = protect_macros(html, attachments)

    print("[4/4] 번역 및 게시 중...")
    for lang in ("ko", "ja"):
        print("      -> " + lang + " 번역 중...")
        translated_html = restore_macros(translate_html(protected_html, glossary, lang), placeholders)
        page_title = translate_title(title, glossary, lang)
        publish_page(PARENT_PAGE_ID[lang], SPACE_KEY, page_title, translated_html)
